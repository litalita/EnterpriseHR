...
import com.wls.mapper.AttendanceMapper;
import com.wls.mapper.EmployeeMapper;
import com.wls.service.AttendanceService;
import com.wls.util.MConstant;
...
public class AttendanceServiceImpl extends ServiceImpl<AttendanceMapper, Attendance> 
	...
	@Override
	public void addStart(Integer employeeNumber,String timeType){
		//获取当前时间
	...
	public Attendance createAttendance(Date nowTime,Date nowDate,Integer employeeNumber,String timeType){
		Attendance attendance = new Attendance();
		attendance.setEmployeeNumber(employeeNumber);
		attendance.setDay(nowDate);
		attendance.setStartTime(nowTime);
		attendance.setTimeType(timeType);
		attendance.setStartType("迟到");
		baseMapper.insertAllColumn(attendance);
		return attendance;
	}
	@Override
	public void addEnd(Integer employeeNumber) {
		...
		if (nowTime.after(amStartTime) && nowTime.before(pmStartTime)) {
			...
			if(attendance==null){
				attendance = createAttendance(nowTime,nowDate,employeeNumber,"上午");
			}
			...
			if(attendance==null){
				attendance = createAttendance(nowTime,nowDate,employeeNumber,"下午");
			}
			if (attendance.getEndTime() == null) {
				attendance.setEndTime(nowTime);
			...
			if(attendance==null){
				attendance = createAttendance(nowTime,nowDate,employeeNumber,"加班");
			}
			if (attendance.getEndTime() == null) {
				attendance.setEndTime(nowTime);
